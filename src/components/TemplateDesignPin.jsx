...
const TemplateDesignPin = ({data, index}) => {
  ...
  {...scaleInOut(index)}
  >
    <div className="w-full h-[500px] 2xl:h-[740px] rounded-md bg-gray-200 overflow-hidden relative" onMouseEnter={()=>setIsHoverred(true)}
        ...
        <AnimatePresence>
            ...
            className="absolute inset-0 bg-[rgba(0,0,0,0.4)] flex flex-col items-center justify-start px-4 py-3 z-50 cursor-pointer">
                <div className="flex flex-col items-end justify-start w-full gap-8">
                    ...
                    <InnerBoxCard label={data?.favourites?.includes(user?.uid) ? "Added to favourites" : "Add to favourites"} 
                    Icon={data?.favourites?.includes(user?.uid) ? BiSolidHeart : BiHeart} onHandle={addToFavourites} />

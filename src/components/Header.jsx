...
const Header = () => {
    ...
    const { data:filterData={ searchTerm: '' } } = useFilters()
    // ={ searchTerm: '' } comment this
    ...
    const handleSearchTerm = (e)=>{     // this whole method commented
  ...
  return <header className="w-full flex items-center justify-between px-4 py-3 lg:px-8 border-b border-gray-300 bg-bgPrimary z-50 gap-12 sticky top-0">
    {/* logo */}
    <Link to={"/"}>
    ...
    <div className="flex-1 border border-gray-300 px-4 py-1 rounded-md flex items-center justify-between bg-gray-200">  
        <input 
        value={filterData?.searchTerm ? filterData?.searchTerm : ""} // comment
        onChange={handleSearchTerm} // comment
        type="text" placeholder="Search here...." className="flex-1 h-10 bg-transparent text-base font-semibold outline-none border-none"></input>

        <AnimatePresence> 
           { filterData?.searchTerm.length > 0 && ( <motion.div 
           onClick={clearFilter}
        ...
        {/* This whole <AnimatePresence comment> */}
    </div>
        ...
        <React.Fragment>
           ...
           onClick={() => setIsMenu(!isMenu)}>
                {data?.photoURL ? <div className="w-12 h-12 rounded-md relative flex items-center justify-center cursor-pointer">
                    <img src={data?.photoURL} className="w-full h-full object-cover rounded-md" referrerPolicy="no-referrer" alt=""></img>
                    ...
                    {data?.photoURL ? <div className="w-20 h-20 rounded-full relative flex items-center flex-col justify-center cursor-pointer">
                    <img 
                        src={data?.photoURL} className="w-full h-full object-cover rounded-full" referrerPolicy="no-referrer" alt="">
